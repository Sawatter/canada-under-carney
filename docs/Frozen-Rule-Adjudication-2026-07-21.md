...
This release does not change edge gestures, pull-to-refresh handling, forced-colors rules, body locking, or sheet overscroll mechanics. The existing iOS edge-swipe/sheet-overscroll, Android pull-to-refresh, and Windows forced-colors checks cannot be completed in this macOS workspace because no iOS simulator, Android SDK/emulator, or Windows browser environment is installed. They remain explicit physical-environment exceptions, not completed checks. Desktop and mobile browser inspection still applies to the added sub-score disclosure content.
