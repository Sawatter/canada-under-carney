...
3. Tiny audit-fix pack
   ...
   - Ethics status mismatch in [docs/Dimension-Status-Register.md](<repo root>/docs/Dimension-Status-Register.md)
...
1. QA gatekeeping review pass
   Run a narrow reflection + review on [docs/QA-Gatekeeping-Rules.md](<repo root>/docs/QA-Gatekeeping-Rules.md) to check whether it still fits the dashboard as it exists now:
   - current scoring workflow
   - confidence calibration
   - source-role discipline
   - probationary dimensions
   Reference the verbatim QA note in [Parking-Lot.md](<repo root>/docs/Parking-Lot.md) so the reasoning is preserved exactly.
