...
| 6 | Perceived-bias testing: real cross-party reader survey measures whether different priors can see how grades were reached. | **Methodology built, activation parked.** The combined reader study in `Current-Roadmap.md` is the activation gate and requires readers with different political priors. |
...
- [ ] Has the source-to-trigger pass been recorded for the cycle? For each of the 11 graded dimensions, one line: trigger fired and grade moved (with source citation) / trigger considered and not fired (with one-line rationale) / trigger source not checked this cycle (with deferral reason). The May 2026 source-to-trigger audit at `docs/Source-To-Trigger-Audit-2026-05.md` is the template. (Added v5.56 after the May audit found 6 of 10 holds had at least one trigger source that was not evaluated in the cycle record. Pulling sources is not the same as evaluating them against pre-committed triggers; the dashboard cannot say it lets evidence move grades unless the per-cycle record shows the evaluation.)
...
7. **Perceived-bias survey mechanism:** methodology and links built in v5.37; activation is parked in the combined reader study in `Current-Roadmap.md`. Companion outreach templates for the audience-validation half live in `docs/Validation-Sprint-Templates.md` (v5.54). The survey collects reader signal; the templates recruit readers with different political priors.
...
- **v1.3 (2026-05-17):** Added the source-to-trigger pass to the pre-cycle checklist after the May 2026 source-to-trigger audit (`docs/Source-To-Trigger-Audit-2026-05.md`) found that 6 of 10 holds in the May cycle had at least one trigger source that was not evaluated in the cycle record. The checklist item asks for one line per graded dimension stating whether the trigger fired, was considered and not fired, or was not checked. Closes the audit gap routinely instead of reconstructing after the fact.
- **v1.4 (2026-08-26):** Mapped perceived-bias survey activation to the parked combined reader study and kept the annual foundational audit due no later than 2027. No audit principle or scoring rule changed.
