...
The Major Projects threshold ladder no longer grades on first-event triggers ("at least one project completes a full MPO cycle"). It now defines the project universe — currently 15 MPO-cohort projects across three tranches (Sept 2025, Nov 2025, Mar 2026) after the May 13 reconciliation to the official MPO list — and grades on % of the cohort that has advanced from `designated` against the new published `stageGates` ladder (designated → reviewed → approved → permitted → under_construction → completed).
...
As of 2026-05-13, 4 of 15 projects sit above designated status, but only 2 of 15 (~13%) show documented post-designation advancement in the recorded cohort data — Contrecœur Terminal under construction since Apr 9 2026; Darlington New Nuclear with a later-dated approval / financing milestone. Mackenzie Valley Highway and North Coast Transmission Line still carry same-day promoted-stage labels from tranche-announcement sources, so they are not yet counted toward the advancement threshold. That sits well below the 30% threshold required for B; the C grade therefore holds, with the credit-claiming penalty modifier still applied.
