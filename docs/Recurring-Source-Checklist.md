...
| Ethics Commissioner reports | `https://ciec-ccie.parl.gc.ca/en/investigations-enquetes/Pages/AllInvestRepAct-TousRapEnqLoi.aspx` | Ethics & Transparency | Automated by `scripts/fetch-data.py` where network-reachable; page diff cached in `tmp/ethics-reports.json` | New report, examination, or PM-relevant filing | Ethics review status changes |
...
6. ~~Ethics Commissioner investigation-report page diff.~~ **Done.** `scripts/fetch-data.py` scrapes the public investigation-report listing page where the network allows it, keeps a small `tmp/ethics-reports.json` cache, and reports additions / removals each cycle. Some local networks still block `ciec-ccie.parl.gc.ca`; those runs surface a fetch failure instead of silently passing.
