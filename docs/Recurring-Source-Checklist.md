...
|---|---|---|---|---|---|
| Fetch script | `python3 scripts/fetch-data.py` | StatCan, IRCC, Bank of Canada | Automated availability check plus StatCan WDS cube metadata where reachable | Endpoint failures, downloaded IRCC CSVs, Bank of Canada response, StatCan `cubeEndDate` freshness flags | Any failure, or any StatCan `cubeEndDate` newer than the dashboard's cited reference period |
| StatCan food CPI | Table / Daily CPI release | Affordability Response | Automated WDS metadata check in script; exact value still editor-reviewed | Latest food-store CPI table freshness | Food CPI crosses trigger or changes current metric |
...
| Excluded-pollster surface | Pollara, Ipsos, Innovative Research Group | Approval Signal | Automated by `scripts/fetch-data.py` (RSS feeds; same federal-relevance filter as cited pollsters; flags as `[REVIEW]`). Mainstreet Research and EKOS Politics remain manual (no public RSS). | New direct Carney / federal-government approval release from a non-cited pollster | Add pollster to cited list if release meets inclusion criteria |
| Policy / academic / journalism surface | C.D. Howe, Fraser Institute, The Hub, Democracy Watch, PROOF, The Narwhal | Independent challenge / context across dimensions | Automated by `scripts/fetch-data.py` (RSS feeds; items tagged `[TOPIC]` vs `[OTHER]`). CCI, IISD, Conversation Canada, Globe and Mail, CBC, National Observer remain manual. | New independent analysis on any dimension theme | Add as evidence source on the affected dimension card |
...
| Ethics Commissioner reports | `https://www.ethicscanada.ca/en/report?type=inv` | Ethics & Transparency | Automated by `scripts/fetch-data.py`; page diff cached in `monitoring/ethics-reports.json` | New report, examination, or PM-relevant filing | Ethics review status changes |
...
| Major Projects project-by-project recertification | MPO page plus each project source URL | Major Projects | All 18 projects, stages, stage dates, and advancement counts; use the current cohort length rather than a historical fixed count |
...
2. ~~StatCan value-diff checks, not just availability checks.~~ **Done at table-freshness level.** `scripts/fetch-data.py` hits the WDS `getCubeMetadata` endpoint for the 5 cited StatCan PIDs and compares `cubeEndDate` against the dashboard's cited reference period. A newer `cubeEndDate` flags a refresh review. Full per-coordinate value extraction still needs a coordinate map per cited series and is a separate future enhancement.
...
6. ~~Ethics Commissioner investigation-report page diff.~~ **Done.** `scripts/fetch-data.py` scrapes the current public report index, keeps a small `monitoring/ethics-reports.json` cache, and reports additions / removals each cycle. Fetch failures surface explicitly instead of silently passing.
