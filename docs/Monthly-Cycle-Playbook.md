...
- **Promise status-source watch list.** All 43 tracked promises now have original-source links, and 40 have `statusSourceUrl`. Keep watching the three remaining gaps — Foreign Policy Review, Red Tape Review, and Carbon Border Adjustment Mechanism — and add a status-evidence link only when a clean official or Tier 2 document appears.
