...
- **Next scheduled cycle:** first day of each month. Next cycle: 2026-10-01 (per `meta.nextUpdate` in [src/data/meta.json](../src/data/meta.json)).
...
- **Freeze window:** when enabled, the source scout runs on the first day. The live update can land after editor review; always update `meta.json` to match reality after the cycle lands.
...
**September 2026 editor exception, approved September 5:** The editor directed,
"Use manual review for September and pause the hosted scout." The next three
scout checks are not applicable for September under that explicit exception,
not passed. The [September cycle report](September-Monthly-Cycle-Report-2026-09-05.md#editor-decisions)
records the decision. Manual authoritative-source discovery and adjudication
replace the scout packet for this cycle. The other evidence, editor, data,
build, browser and review gates still apply. Failed scout artifacts remain
excluded. The hosted workflow is paused and hosted acceptance remains open.

**September closure exception, September 6:** Under the user's delegated editorial authority, the analyst retained the prior Affordability display for this cycle only. The grade-moving coverage gap remains unestablished; it is not ordinary carry-forward or a passed evidence test. The [dated exception](September-Affordability-Exception-2026-09-06.md) records the scope and October 1 expiry. This cycle-specific exception does not amend scoring thresholds, formulas, weights or the standing E4 rule. The analyst also completed the Housing party-symmetry check under delegation, without attributing a personal belief to the editor.
...
- [ ] Re-check the currently excluded pollsters in the Approval Signal decision memo for a direct approve/disapprove release since last cycle. Research Co. entered the September cycle through the existing equivalent-transparency route for its broad job-approval wave; issue-specific tariff questions remain excluded. Apply the current memo to each new wave.
...
- [ ] Set `coveragePeriod.end` to the actual evidence cutoff covered by the review. For a regular monthly cycle, use the final day of the prior calendar month. Keep the review/publication date in `lastUpdated`; do not extend evidence coverage merely because review finishes later.
