...
- [ ] Add `changes[]` entries for every grade move, metric update, event, and launch.
- [ ] Grade changes must reference the dim's `dimensionId` and include a plain-language `reason`.
...
- [ ] Eyeball the built dashboard: the WhatsChanged box should show the new changelog entry; the ApprovalSignal should show the updated rolling window; score cards should reflect the new grades.
