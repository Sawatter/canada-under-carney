...
- **Named exceptions are explicitly framed:**
  ...
  - Flagship Delivery is a meta-rollup, not a peer dimension. Scoped explicitly in its `judgmentCall`. It exited probation after the July 2026 retention check (v5.150, recorded in `docs/Flagship-Delivery-Rules.md`): the Combination Rule ran unchanged, the direction did not contradict the underlying file-status movement, the cycle yielded a cross-file execution insight spanning two or more flagship files, and no unresolved deconfliction breach was found. Those four checks now act as retention conditions. If a future cycle fails one, queue an explicit editor decision on replacement or demotion rather than reviving probation. The closed 2026 tri-lens pilot is not an active destination.
